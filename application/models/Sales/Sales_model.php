...
//            $this->db->order_by("sales_no", "desc");
    ...
    function getSalesReport($headers)
    {
        if((!$_POST['login_user']) && (($_POST['login_user'] == null)))
        {
            ...
            {
                // print_r($_POST);die();
                $query11 = $this->db->query("select b.ino,a.iname,sum(quan) as quan ,sum(amount) as amount,category,item_type 
    ...
    function getBill($headers){
        ...
        } else if(empty($_POST['billno']))
        {
            ...
            $data['sales'] = empty($userData)?'':$userData[0];
    ...
    function getRetriveBill($headers)
    {
        if((!$_POST['login_user']) && (($_POST['login_user'] == null)))
        {
            ...
            // $agent = $userData?$userData[0]['acode']:'';
            
            // $this->db->where('acode', $user);
            // $shop =  $this->db->get('agent');
            
            // $data['agent'] = array();
            // if($shop->result_array())
            //     $data['agent'] = $shop->result_array()[0];
                
            
            $this->db->delete('sales_hold', array('billno' => $_POST['viewBill']));
...
//             $this->db->join('master_customer', 'master_sales.cid= master_customer.cid');
            ...
            // $this->db->where('stock.branch',$_POST['branch']);
             $this->db->order_by('stock.dat', "desc");
            $this->db->where('stock.quan >=', '1');
        ...
        if((!$_POST['login_user']) && (($_POST['login_user'] == null)))
        {
           ...
           date_default_timezone_set('Asia/Kolkata');
            ...
            if($_POST['type'] == 'alter' )
            {
                ...
                $this->db->delete('sales_items', array('billno' => $_POST['billno'])); 
            }
            ...
            foreach($stock as $data)
            {
                ...
                } else if($_POST['type'] == 'alter' )
                {
                    ...
                    $this->db->insert('sales_items', $insertData);
                } else {
            ...
            if($_POST['type'] == 'hold' )
            {
                $data =  $this->db->insert('sales_hold', $insertData);
            } 
            else if($_POST['type'] == 'alter' )
            {
                ...
                $this->db->where('billno', $_POST['billno']);
                $data =  $this->db->update('sales', $insertData);
            } else {
                
                $billno = $lot_no;
                
                $data =  $this->db->insert('sales', $insertData);
            }
    ...
    function deleteSales($headers){
        ...
        } else if((!$_POST['billno']) && (($_POST['billno'] == null)))
        {
            ...
            if($billData)
            {
                ...
                $data = $this->db->delete('sales', array('billno' => $_POST['billno']));
                $this->db->delete('sales_items', array('billno' => $_POST['billno']));
